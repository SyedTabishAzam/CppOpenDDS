...
DataReaderListenerImpl::on_data_available(DDS::DataReader_ptr reader)
{
  ...
  if (error == DDS::RETCODE_OK) {
	...
	string line;
	fstream myfile;
	myfile.open("S:\\Presagis\\Suite16\\STAGE\\GUIPlugin\\entitymission.missionactive", ios::out | ios::app);
    ...
    if (info.valid_data) {
		...
		{
			myfile << "NEW_COMMAND " << message.entityname.in() << " " << message.key.in()<<" "<< message.value.in() << std::endl;
		}
